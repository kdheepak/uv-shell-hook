...
class TestBashHook:
    ...
    def test_activate_virtual_environment(self, temp_project: Path, bash_hook_file: Path):
    ...
    def test_deactivate_virtual_environment(self, temp_project: Path, bash_hook_file: Path):
    ...
    def test_activate_nonexistent_environment(self, temp_project: Path, bash_hook_file: Path):
    ...
    def test_deactivate_when_none_active(self, temp_project: Path, bash_hook_file: Path):
        ...
        assert ("No virtual environment is active" in result.stderr or 
                "deactivate function not available" in result.stderr)
    
    def test_regular_uv_commands_passthrough(self, temp_project: Path, bash_hook_file: Path):
    ...
    def test_python_path_in_activated_environment(self, temp_project: Path, bash_hook_file: Path):
        """Test that Python path points to the virtual environment when activated."""
        result = run_bash_with_hook(
            "uv activate && which python",
            temp_project,
            bash_hook_file
        )
    ...
    def test_deactivation_success_message(self, temp_project: Path, bash_hook_file: Path):
